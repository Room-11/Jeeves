...
class Mdn implements Plugin {
    ...
    private function getResult(Message $message): \Generator
    {
        $response = yield from $this->chatClient->request(
            'https://developer.mozilla.org/en-US/search.json?q=' . rawurlencode(implode('%20', $message->getParameters()))
        );
        $result = json_decode($response->getBody(), true);
        $firstHit = reset($result->documents[0]);
        if(isset($firstHit->id) && isset($firstHit->url)) {
